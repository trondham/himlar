...
class profile::openstack::dashboard(
  ...
  # Congress: Install the Congress plugin (RPM packages) for Horizon
  # if "enable_congress" is set to true
  if $enable_congress {
    $congress_packages = lookup('profile::openstack::dashboard::congress_packages', Hash, 'deep', {})
    create_resources('profile::base::package', $congress_packages)
  }
